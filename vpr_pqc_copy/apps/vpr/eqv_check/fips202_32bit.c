...
uint32_t three_xor_count = 0;
uint32_t rol64_count = 0;
uint32_t bcop_count = 0;

uint32_t three_xor_avg_cyc = 0;
uint32_t three_xor_tot_cyc = 0;
uint32_t rol64_avg_cyc = 0;
uint32_t rol64_tot_cyc = 0;
uint32_t bcop_avg_cyc = 0;
uint32_t bcop_tot_cyc = 0;

static inline void rol64_concat(uint32_t in_lo, uint32_t in_hi, uint32_t offset, uint32_t *out_lo, uint32_t *out_hi)
{
  //uint32_t cyc1 = cpucycles();
  #ifndef STD_ROL64
  //rol64_count += 2;
  uint32_t custom_lo, custom_hi;
  asm volatile(
      ".insn r CUSTOM_0, 0x2, 0x0, %[lo_bits], %[in_lo], %[in_hi], %[offset]\n\t"
      "nop\n\t"
      ".insn r CUSTOM_0, 0x2, 0x1, %[hi_bits], %[in_lo], %[in_hi], %[offset]"
        : [lo_bits] "=&r" (custom_lo), [hi_bits] "=&r" (custom_hi)
 ...
 /* 
  ...
  volatile uint64_t concat_val = ((uint64_t)in_hi << 32) | in_lo;
  ...
  *out_hi = std_hi;
  #endif
  /*
  uint32_t cyc2 = cpucycles();
  uint32_t diff = cyc2 - cyc1;
  if(rol64_avg_cyc != 0){
    rol64_avg_cyc = (rol64_avg_cyc + diff)/2;
  } else {
    rol64_avg_cyc = diff;
  }
  rol64_tot_cyc += diff; 
...
static inline void rv_3xor(uint32_t rs1, uint32_t rs2, uint32_t rs3, uint32_t *rd)
{
  ...
  *rd = rs1 ^ rs2 ^ rs3;
  ...
  uint32_t cyc2 = cpucycles();
  uint32_t diff = cyc2 - cyc1;
  if(three_xor_avg_cyc != 0){
    three_xor_avg_cyc = (three_xor_avg_cyc + diff)/2;
  } else {
    three_xor_avg_cyc = diff;
  }
  three_xor_tot_cyc += diff;
  */
}
...
static inline void rv_bcop(uint32_t rs1_lo, uint32_t rs1_hi, uint32_t rs2_lo, uint32_t rs2_hi, uint32_t rs3_lo, uint32_t rs3_hi, uint32_t *rd_lo, uint32_t *rd_hi)
{
  ...
  *rd_hi = rs1_hi ^ ((~rs2_hi) & rs3_hi);
  ...
  uint32_t diff = cyc2 - cyc1;
  if(bcop_avg_cyc != 0){
    bcop_avg_cyc = (bcop_avg_cyc + diff)/2;
  } else {
    bcop_avg_cyc = diff;
  }
  bcop_tot_cyc += diff;
...
static const uint32_t KeccakF_RoundConstants32[2 * NROUNDS] = {
...
static inline uint32_t five_input_xor(uint32_t a, uint32_t  b,uint32_t c,uint32_t d, uint32_t e)
{
  uint32_t temp, res;
  
  rv_3xor(a, b, c, &temp);
  rv_3xor(temp, d, e, &res);
...
void KeccakF1600_StatePermute_32bit(uint32_t *state) {
    ...
    uint32_t Do_lo_tmp, Do_hi_tmp;
    uint32_t Du_lo_tmp, Du_hi_tmp;
    uint32_t BCa_lo_tmp, BCa_hi_tmp;
    ...
    for(round = 0; round < NROUNDS; round += 2) {
      #ifndef STD_5XOR
      BCa_lo = five_input_xor(Aba_lo, Aga_lo, Aka_lo, Ama_lo, Asa_lo);
      BCa_hi = five_input_xor(Aba_hi, Aga_hi, Aka_hi, Ama_hi, Asa_hi);

      BCe_lo = five_input_xor(Abe_lo, Age_lo, Ake_lo, Ame_lo, Ase_lo);
      BCe_hi = five_input_xor(Abe_hi, Age_hi, Ake_hi, Ame_hi, Ase_hi);

      BCi_lo = five_input_xor(Abi_lo, Agi_lo, Aki_lo, Ami_lo, Asi_lo);
      BCi_hi = five_input_xor(Abi_hi, Agi_hi, Aki_hi, Ami_hi, Asi_hi);

      BCo_lo = five_input_xor(Abo_lo, Ago_lo, Ako_lo, Amo_lo, Aso_lo);
      BCo_hi = five_input_xor(Abo_hi, Ago_hi, Ako_hi, Amo_hi, Aso_hi);

      BCu_lo = five_input_xor(Abu_lo, Agu_lo, Aku_lo, Amu_lo, Asu_lo);
      BCu_hi = five_input_xor(Abu_hi, Agu_hi, Aku_hi, Amu_hi, Asu_hi);
      ...
      rol64_concat(BCi_lo, BCi_hi, 1, &BCi_lo_tmp, &BCi_hi_tmp);
      De_lo = BCa_lo ^ BCi_lo_tmp;
      De_hi = BCa_hi ^ BCi_hi_tmp;

      rol64_concat(BCo_lo, BCo_hi, 1, &BCo_lo_tmp, &BCo_hi_tmp);
      Di_lo = BCe_lo ^ BCo_lo_tmp;
      Di_hi = BCe_hi ^ BCo_hi_tmp;

      rol64_concat(BCu_lo, BCu_hi, 1, &BCu_lo_tmp, &BCu_hi_tmp);
      Do_lo = BCi_lo ^ BCu_lo_tmp;
      Do_hi = BCi_hi ^ BCu_hi_tmp;

      rol64_concat(BCa_lo, BCa_hi, 1, &BCa_lo_tmp, &BCa_hi_tmp);
      ...
      BCu_lo = BCu_lo_tmp;
        ...
        : [rs1_lo] "r" (BCa_lo), [rs2_lo] "r" (BCe_lo), [rs3_lo] "r" (BCi_lo),
          [keccak_lo] "r" (KeccakF_RoundConstants32[round * 2 + 1]),
          [rs1_hi] "r" (BCa_hi), [rs2_hi] "r" (BCe_hi), [rs3_hi] "r" (BCi_hi),
          [keccak_hi] "r" (KeccakF_RoundConstants32[round * 2])
          : "a3","a5", "cc"
        );
      /*
      #elif defined(CUSTOM_BCOP_XOR_LMW)
        asm volatile(
      ...
      */
        ...
        rv_bcop(BCa_lo, BCa_hi, BCe_lo, BCe_hi, BCi_lo, BCi_hi, &Eba_lo, &Eba_hi);
        Eba_lo ^= KeccakF_RoundConstants32[round * 2 + 1];
        Eba_hi ^= KeccakF_RoundConstants32[round * 2];
      #endif
      ...
      rv_bcop(BCu_lo, BCu_hi, BCa_lo, BCa_hi, BCe_lo, BCe_hi, &Ebu_lo, &Ebu_hi);
      ...
      #ifndef STD_5XOR
      BCa_lo = five_input_xor(Eba_lo, Ega_lo, Eka_lo, Ema_lo, Esa_lo);
      BCa_hi = five_input_xor(Eba_hi, Ega_hi, Eka_hi, Ema_hi, Esa_hi);

      BCe_lo = five_input_xor(Ebe_lo, Ege_lo, Eke_lo, Eme_lo, Ese_lo);
      BCe_hi = five_input_xor(Ebe_hi, Ege_hi, Eke_hi, Eme_hi, Ese_hi);

      BCi_lo = five_input_xor(Ebi_lo, Egi_lo, Eki_lo, Emi_lo, Esi_lo);
      BCi_hi = five_input_xor(Ebi_hi, Egi_hi, Eki_hi, Emi_hi, Esi_hi);

      BCo_lo = five_input_xor(Ebo_lo, Ego_lo, Eko_lo, Emo_lo, Eso_lo);
      BCo_hi = five_input_xor(Ebo_hi, Ego_hi, Eko_hi, Emo_hi, Eso_hi);

      BCu_lo = five_input_xor(Ebu_lo, Egu_lo, Eku_lo, Emu_lo, Esu_lo);
      BCu_hi = five_input_xor(Ebu_hi, Egu_hi, Eku_hi, Emu_hi, Esu_hi);
      #else
      ...
      Aba_lo ^= KeccakF_RoundConstants32[round * 2 + 3];
      Aba_hi ^= KeccakF_RoundConstants32[round * 2 + 2];
